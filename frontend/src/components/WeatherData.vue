<template>
  <div
    ...
    class="flex flex-col items-center justify-center text-gray-700 p-10 bg-gradient-to-br from-pink-200 via-purple-200 to-indigo-200"
  >
    <div
    ...
    >
      <div class="flex justify-between">
        ...
        <div>
            <img
                class="mx-auto h-24 w-24 fill-current text-yellow-400"
                :src="icon"
            />
            <h3 class="text-center text-base font-semibold leading-6 text-gray-900">{{ description }}</h3>
        </div>
      </div>
...
weatherStore.fetchWeather(props.user.id)
...
const location = computed(() => {
    return weatherStore.weather.name + ', ' + weatherStore.weather['sys'].country
})

const icon = computed(() => {
    return 'http://openweathermap.org/img/wn/' + weatherStore.weather['weather'][0].icon + '@2x.png';
})
